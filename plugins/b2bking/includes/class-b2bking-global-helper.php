...
class B2bking_Globalhelper{
	...
	public static function check_visibility_cache_integrity($cache_product_ids) {
	    ...
	    // Get current count to check against log
	    $product_ids_count = count($cache_product_ids);
	    
	    // CRITICAL CHECK 1: If zero or very few products, almost certainly corrupt cache
	    $min_expected_products = intval(get_option('b2bking_min_expected_products', 100));
	    if ($product_ids_count < $min_expected_products) {
	        return false;
	    }
	    
	    // CRITICAL CHECK 2: Check if we're on a shop/archive page with no products
	    // This detects the "no products found" condition
	    ...
	    // Continue with your existing max check
	    ...
	    // Simply get the highest number from the log
	    $highest_count = max($user_cache_integrity_log);
	    
	    // If difference is more than 10 items, invalidate
	    if (abs($product_ids_count - $highest_count) > 10) {
	        if (defined('WP_DEBUG') && WP_DEBUG) {
	            error_log("B2BKing cache integrity fail: User $currentuserid has $product_ids_count products but highest recorded count is $highest_count");
	        }
	...
	*/
	...
	public static function get_credit_gateway_content(){
		...
		if ($available_credit >= $cart_total){
			?>
			<p class="form-row"><?php echo esc_html__('Available credit: ','b2bkingcredit').wc_price($available_credit);?></p>
			<?php
		} else {
			?>
			<p class="form-row"><?php echo esc_html__('Insufficient credit - your available credit is: ','b2bkingcredit').wc_price($available_credit);?></p>
			<?php
			if (intval(get_option( 'b2bking_allow_redeem_setting', 1 )) === 1){
				...
				echo esc_html__('You can ','b2bkingcredit'); 
				echo '<a target="_blank" href="'.trailingslashit(get_permalink( get_option('woocommerce_myaccount_page_id') ) ).get_option('b2bking_credit_endpoint_setting', 'company-credit').'">'.esc_html__('redeem credits','b2bkingcredit').'</a>';
				echo esc_html__(' as a coupon.','b2bkingcredit');
