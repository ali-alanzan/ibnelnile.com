...
export default {
  ...
  watch: {
    ready: {
      handler(newValue, oldValue) {
        this.uipApp.fathomAnalytics.ready = this.returnStatus;
      },
      deep: true,
    },
  ...
  methods: {
    ...
    async pushToGlobal() {
      ...
      this.uiTemplate.fathomAnalytics.api = this.uipFathomAnalytics;
      this.uiTemplate.fathomAnalytics.ready = this.returnStatus;

      this.uipApp.fathomAnalytics = {
        api: this.uipFathomAnalytics,
     ...
     * @since 3.0.0
     */
    async uipFathomAnalytics(action, startDate, endDate) {
      ...
      if (!this.hasAccount) {
        let error = {};
        error.error = true;
        error.message = __('No fathom account connected', 'uipress-pro');
    ...
    async getQueryURL(resolve) {
      ...
      formData.append('action', 'uip_build_fathom_analytics_query');
      ...
      // handle error
      if (response.error) {
        //No fathom account connected
        if (response.error_type == 'no_fathom') {
          this.hasAccount = false;
          this.queryURL = 'no_fathom';
        }
      ...
      this.fathomPath = response.fathomPath;
      if (typeof resolve !== 'undefined') {
        resolve(true);
      }
    ...
    async runReport(dates, resolve, lastresolve) {
      ...
      if (response.error) {
        ...
        let errorTitle = __('Unable to fetch analytic data', 'uipress-lite');
      ...
      response.fathomPath = this.fathomPath;
      resolve(true);
      lastresolve(response);
    },
    ...
    async saveToken(token) {
      if (!token || token == this.toke) return;

      let formData = new FormData();
      formData.append('action', 'uip_save_fathom_access_token');
      formData.append('security', uip_ajax.security);
    ...
    async removeAnalyticsAccount(resolve) {
      let self = this;
      let formData = new FormData();
      formData.append('action', 'uip_remove_fathom_analytics_account');
      formData.append('security', uip_ajax.security);
      formData.append('saveAccountToUser', this.saveAccountToUser);
