...
class DUP_PRO_Package_Template_Entity extends AbstractEntityList implements UpdateFromInputInterface
{
    ...
    public static function createFromImportData(self $data)
    {
        ...
        if (count($instance->components) > 0) {
            return $instance;
        }

        //Allow import of older templsates that did not have package components
        if ($instance->archive_export_onlydb) {
            $instance->components = [BuildComponents::COMP_DB];
        } else {
            $instance->components = BuildComponents::COMPONENTS_DEFAULT;
        }
